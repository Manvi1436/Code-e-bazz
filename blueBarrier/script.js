...
(function () {
  if (!document.getElementById('crMap')) return; // only run if section exists
  ...
  function getFeaturesForNode(nodeId, dayIdx, r) {
    ...
    return {
      wind: Math.round(20 + 60*base + dayIdx*0.7),
      tide: +(0.5 + 2.8*base).toFixed(2),
      humidity: Math.round(60 + 40*base),
      stormProb: +(0.05 + 0.6*base).toFixed(2)
    };
  }
  function riskScore(f){
    const w = f.wind/100, t = f.tide/3.5, h = f.humidity/100, s = f.stormProb;
    return Math.min(1, 0.4*w + 0.3*t + 0.2*h + 0.1*s);
  }
  ...
  function makeForecast(node){
    ...
    for (let i=0;i<7;i++){
      ...
      const s = riskScore(f);
      const b = band(s);
      days.push({ day: new Date(Date.now()+ i*86400000), band: b, color: colorFor(b), drivers: f, score: +s.toFixed(2) });
    }
    return days;
  }
  function makeHistory(node){
    ...
    const notes  = ["Wind ↑ & Tide ↑","Humidity ↑, Wind ↑","Tide peak ~midnight","Unusual swell","Monsoon + rough sea"];
    const rows = [];
    for (let k=0;k<6;k++){
      const d = new Date(Date.now() - (k+1)*7*86400000);
      rows.push({ date: d.toISOString().slice(0,10), label: labels[int(r()*labels.length)], note: notes[int(r()*notes.length)] })
    }
    return rows;
  ...
  function renderSelection(node, latlng){
    ...
    if (latlng){
      ...
      window.__crClickMarker = L.circleMarker(latlng, {
  ...
  COAST_NODES.forEach(n=>{
    ...
    L.circleMarker([n.lat, n.lng], {
      radius: 6, weight: 1.5, color: colorFor(todayBand), fillColor: colorFor(todayBand), fillOpacity: .6
    })
    .addTo(markersLayer)
  ...
  map.on('click', (e)=>{
    const near = nearestNode(e.latlng.lat, e.latlng.lng);
    if (near && near.distKm <= 120) renderSelection(near.node, e.latlng);
    else {
  ...
  async function locateByPincode(pin){
    ...
    if (near && near.distKm <= 120) renderSelection(near.node, [latNum, lonNum]);
    else {
  ...
  const pinBtn = document.getElementById('crFindBtn');
  const pinInput = document.getElementById('crPincode');
  if (pinBtn && pinInput){
    pinBtn.addEventListener('click', async ()=>{
      const pin = (pinInput.value||'').trim();
      if (pin.length !== 6) { alert("Enter a valid 6-digit PIN code"); return; }
      try { await locateByPincode(pin); } catch { alert("Could not locate that PIN code. Try another."); }
    });
  }
